...
// Get longest film
tApiError api_getLongestFilm(tApiData data, tCSVEntry *film) {
    printf("[DEBUG: EX1_2/3] Entering api_getLongestFilm\n");
    printf("[DEBUG: EX1_2/3] Number of films in catalog: %d\n", film_catalog_len(data.films));
    
    if (film == NULL) {
        printf("[DEBUG: EX1_2/3] Output parameter is NULL\n");
        return E_INVALID_ENTRY_TYPE;
    }
    ...
    if (longest == NULL) {
        printf("[DEBUG: EX1_2/3] No films found\n");
        return E_NOT_IMPLEMENTED;
    }
    
    // Convert tFilm to tCSVEntry
    char buffer[FILE_READ_BUFFER_SIZE];
    film_get(*longest, buffer);
    csv_parseEntry(film, buffer, "FILM");
    
    printf("[DEBUG: EX1_2/3] Longest film found: %s, duration: %d:%d\n", 
           longest->name, longest->duration.hour, longest->duration.minutes);
    printf("[DEBUG: EX1_2/3] Exiting api_getLongestFilm with success\n");
    
    return E_SUCCESS;
}

// Get longest free film
tApiError api_getLongestFreeFilm(tApiData data, tCSVEntry *film) {
    printf("[DEBUG: EX1_4/5] Entering api_getLongestFreeFilm\n");
    printf("[DEBUG: EX1_4/5] Number of free films in catalog: %d\n", film_catalog_freeLen(data.films));
    
    if (film == NULL) {
        printf("[DEBUG: EX1_4/5] Output parameter is NULL\n");
        return E_INVALID_ENTRY_TYPE;
    }
    ...
    if (longest == NULL) {
        printf("[DEBUG: EX1_4/5] No free films found\n");
        return E_NOT_IMPLEMENTED;
    }
    ...
    printf("[DEBUG: EX1_4/5] Longest free film found: %s, duration: %d:%d\n", 
           longest->name, longest->duration.hour, longest->duration.minutes);
    printf("[DEBUG: EX1_4/5] Exiting api_getLongestFreeFilm with success\n");
...
tApiError api_sortCatalogByYear(tApiData* data) {
    printf("[DEBUG: EX1_6/7] Entering api_sortCatalogByYear\n");
    
    if (data == NULL) {
        printf("[DEBUG: EX1_6/7] Input parameter is NULL\n");
        return E_INVALID_ENTRY_TYPE;
    }
    
    printf("[DEBUG: EX1_6/7] Number of films in catalog: %d\n", film_catalog_len(data->films));
    printf("[DEBUG: EX1_6/7] Number of free films in catalog: %d\n", film_catalog_freeLen(data->films));
    
    if (film_catalog_len(data->films) == 0 && film_catalog_freeLen(data->films) == 0) {
        printf("[DEBUG: EX1_6/7] Catalog is empty\n");
        return E_NOT_IMPLEMENTED;
    }
    
    // Sort both regular and free film lists
    filmList_sortByDate(&(data->films.filmList));
    freeFilmList_sortByDate(&(data->films.freeFilmList));
    
    printf("[DEBUG: EX1_6/7] Exiting api_sortCatalogByYear with success\n");
    return E_SUCCESS;
}

// Get longest film
tApiError api_getOldestFilm(tApiData data, tCSVEntry *entry, bool free) {
    printf("[DEBUG: EX1_3] Entering api_getOldestFilm (free=%d)\n", free);
    printf("[DEBUG: EX1_3] Number of films in catalog: %d\n", film_catalog_len(data.films));
    printf("[DEBUG: EX1_3] Number of free films in catalog: %d\n", film_catalog_freeLen(data.films));

    if (entry == NULL) {
        printf("[DEBUG: EX1_3] Output parameter is NULL\n");
        return E_INVALID_ENTRY_TYPE;
    }
    ...
    if (oldest == NULL) {
        printf("[DEBUG: EX1_3] No films found\n");
        return E_NOT_IMPLEMENTED;
    }
    ...
    printf("[DEBUG: EX1_3] Oldest film found: %s (%d)\n", oldest->name, oldest->release.year);
    printf("[DEBUG: EX1_3] Exiting api_getOldestFilm with success\n");
